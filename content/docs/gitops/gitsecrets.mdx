...
Once you have a Kubernetes App configured, you are likely to store its YAML files in a git repository for version control purposes.
However, we encounter a problem with secrets. Storing secret YAMLs in git is a bad idea because if someone gains access to our
git repository, they will be able to view the secrets in plaintext.

This article explains how to store Kubernetes Secrets in git securely.

## Prerequisites

- A namespace on your cluster
- kubectl
- kubeseal
...
Now you can apply your sealed secret using `kubectl` as follows:
