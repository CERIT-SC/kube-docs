...
This guide outlines the steps required to set up continuous integration and deployment (CI/CD) between GitLab and Kubernetes. The CI/CD process is managed by the GitLab Kubernetes Agent, which operates within the target namespace—that is, the namespace where CI/CD actions are executed.
...
Conceptually, the GitLab Kubernetes Agent functions as a proxy between GitLab (more precisely, the GitLab Runner that executes CI/CD pipeline jobs) and the Kubernetes API. The agent uses a Kubernetes Service Account to authenticate with the Kubernetes API.

This Service Account must be granted adequate permissions to perform CI/CD operations. These permissions are defined via Kubernetes RBAC resources: `Role` and `RoleBinding`.

## Use Case: Deploying a Helm Chart

CI/CD pipelines can automate a wide range of tasks. This guide focuses on the most common scenario: deploying a Helm chart from a Git repository to a Kubernetes namespace.
...
Log in to GitLab, navigate to your repository, and go to `Operate` → `Kubernetes clusters`. Select **Connect a cluster**, as shown in the following screenshot:

![gitlab1](/img/ceritsc/gitops/gitlab1.png)

Ignore the `Flux` section and enter a name for the agent, for example, `my-agent`.

On the next screen, note the access token and the initial `helm` setup commands (the first two lines).
...
This error indicates a missing role configuration, such as:
```yaml
- apiGroups: ["policy"]
  resources: [poddisruptionbudgets]
  verbs: [create, get, delete, patch]
...
You can identify the required `apiGroups` (shown in YAML files as `apiVersion`) and `resources` (shown as `kind`) from your Helm chart by running the following command in your local Git repository:
```bash
...
This message is normal and expected. It indicates that the Flux module is not in use because FluxCD is not installed in the cluster.
...
Check the job logs in GitLab after the pipeline runs to see the available contexts and verify that the correct one is selected.

Saving this file should automatically trigger a CI/CD pipeline in GitLab. If the job fails due to RBAC permissions (similar to the errors described earlier), update your `role.yaml` file with the necessary permissions and reapply it:
