...
You can identify required `apiGroups` and `resources` from your Helm chart using the following command in your local Git repository:
```bash
for i in `find . -type f -name '*.yaml'`; do grep '^apiVersion:' $i | sort -u; grep '^kind:' $i | sort -u; done
```

<Callout type="info">
If `apiVersion` is simply `v1` or `v2`, the corresponding `apiGroup` is an empty string: `[""]`.
</Callout>
