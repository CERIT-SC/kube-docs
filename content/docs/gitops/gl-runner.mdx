...
GitLab runner is used to run jobs listed in `.gitlab-ci.yaml` in the GitLab repositories. GitLab runner differs from the GitLab Kubernetes agent, which deploys containers into Kubernetes clusters for e2e tests, CI/CD, and other purposes. 
...
To interact with the cluster, you need to install `kubectl` tool and download your `kubeconfig` file. Follow the instructions on how to obtain the file [here](../kubernetes/kubectl).

## Helm - Package Manager for Kubernetes

GitLab runner is distributed as a Helm package. Helm is a package manager for Kubernetes and to be able to use Helm packages, you need to install `helm` command line tool. See official instructions [here](https://helm.sh/docs/intro/install/)

## Kubeseal - Storing Encrypted Secrets

To safely store GitLab tokens, use Sealed Secrets. Install `kubeseal`, the instructions can be found in the offical [github](https://github.com/bitnami-labs/sealed-secrets). More resources about `kubeseal` can be in the git secrets section in [our documentation](./gitsecrets).
...
You should update the GitLab runner version to the newest available or to the version matching the version of the GitLab instance. It might happen that an image is not available for the latest version. In that case, installation will run, but the gitlab-runner in Rancher will be in the state `Imagepullbackoff`, which means that the image does not exist. You may need to adjust values for CPU, memory and concurrent throughout the file in order to fit into your project's quotas.
...
If there a Pod is in `ImagePullbackOff` state, it means that the image either has wrong name, tag, is not pushed to the registry or generally doesn't exist. The image is specified in `values.yaml`, section `image:` so you can check the tag and verify on the [official site](https://gitlab.com/gitlab-org/ci-cd/gitlab-runner-ubi-images/container_registry/1766421) that the tag exists. If not, choose some tag. 
...
The helm chart does not directly support setting the required security parameters of our clusters ([security context, non-root user](../kubernetes/securitycontext)) directly, they need to be overridden using a beta feature called [pod_spec](https://docs.gitlab.com/runner/executors/kubernetes/index.html#overwrite-generated-pod-specifications). The downside is that each container needs to be patched individually, so there needs to be a fixed number of containers.
