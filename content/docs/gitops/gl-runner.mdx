...
Throught these instructions, you need to replace `<YOUR NAMESPACE>` with the name of your personal or group namespace. You can chcek what namespaces are available to you via [Rancher GUI](../kubernetes/ns).

## Kubectl - Interaction with the Cluster

To interact with the cluster, you need to install `kubectl` tool and dowload your `kubeconfig` file. Follow the instructions on how obtain the file [here](../kubernetes/kubectl).
...
If there a Pod is in `ImagePullbackOff` state, it means that the image either has wrong name, tag, is not pushed to the registry or genereally doesn't exist. The image is specified in `values.yaml`, section `image:` so you can check the tag and verify on the [official site](https://gitlab.com/gitlab-org/ci-cd/gitlab-runner-ubi-images/container_registry/1766421?after=MjAyNC0xMC0yMlQyMDoxMjowOS45NTU1MThafGFtZDY0LXYxNy4yLjM%3D) that the tag exists. If not choose, some tag. 
...
The helm chart does not directly support setting the required security parameters of our clusters ([security context, non-root user](../kubernetes/securitycontext)) directly, they need to be overriden using a beta feature called [pod_spec](https://docs.gitlab.com/runner/executors/kubernetes/index.html#overwrite-generated-pod-specifications). The downside is that each container needs to be patched individually, so there needs to be a fixed number of containers.
