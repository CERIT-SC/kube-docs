...
Harbor is a multi-user docker registry with a web UI available at [hub.cerit.io](https://hub.cerit.io). The main advantage of having our own registry as opposed to using e.g. [hub.docker.com](https://hub.docker.com) is that we can offer unlimited private repositories to our users and thus they can create as many docker images as they want without publicly exposing them to the world.
