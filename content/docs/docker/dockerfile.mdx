...
A Docker image is an essential part of container infrastructure. It is an image that contains an application and all its required parts (libraries and other files).

A Docker image is usually created using a `Dockerfile`. The `Dockerfile` contains commands to build the image, environment variables, a command to run when the container starts, an entry point, and many other items.
...
Before building your own Docker image, we strongly recommend searching the public registry for an existing image that contains your desired application. Also check the timestamp of the last image to see whether someone is actively maintaining it with new releases and bug fixes. For example, we recommend Bitnami images. Be aware of some limitations when using external Docker images on our infrastructure; see [limitations](./limitations).
...
<Callout type="warning">Cleaning must be done within a single **RUN** command.</Callout>
...
The Docker image can be built either manually or by automated build tools.
...
To build a Docker image locally, you need a machine running Linux with Docker installed. To install Docker, follow the instructions for your Linux distribution; usually `docker-ce` is the right package. You may need to become a member of the `docker` group to use Docker.

Put the `Dockerfile` in a directory, for example, `mydocker`, and run the following, replacing `repo/name:tag` with the correct name and tag:
```bash
docker build -t repo/name:tag mydocker
```

If the build succeeds, the image is built locally and needs to be pushed to a registry. If you are not logged in to the registry, issue `docker login registry`, replacing `registry` with the real registry name:
```bash
...
Automated builds can be done using a CI/CD pipeline. The build is done using the *runner* component, which is part of many GitHub/GitLab installations. You can use [gitlab.ics.muni.cz](https://gitlab.ics.muni.cz); see [GitLab Containers](../gitops/building_containers) for instructions. This GitLab also provides a Docker registry, so once built, the image can be stored here for use from Kubernetes. The registry is public or private depending on the project settings; see [GitLab Containers](../gitops/building_containers#how-to-use-images-from-gitlab-registry).

The automated build can be slow, so it is not the best option for debugging the build process.

## Running Docker Images

Manually running the built image before running it on Kubernetes can be useful, for example, to check that all files are in place, important directories are writable, and so on. Running it manually is fast and has no privilege restrictions. However, be careful about running unknown images on your machine. To run a Docker image locally, you need a Linux machine with Docker installed; see the manual build section above.
...
This command will run the `registry/name:tag` image and run the `command` inside it. You can add `-u uid` to run the image as user `uid`.
...
Pressing `ctrl-c` or `ctrl-d` will kill the running Docker image. All changes inside the container will be lost unless you use `docker commit`; see the [official Docker documentation](https://docs.docker.com/build/). In this case, do not use the `--rm` option, which automatically removes terminated containers.
