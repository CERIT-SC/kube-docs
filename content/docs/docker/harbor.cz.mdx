...
Harbor je vícerozměrný docker registry s webovým uživatelským rozhraním dostupným na [hub.cerit.io](https://hub.cerit.io). Hlavní výhodou vlastního registru oproti používání např. [hub.docker.com](https://hub.docker.com/) je, že můžeme našim uživatelům nabídnout neomezené soukromé repozitáře, a tím pádem mohou vytvářet tolik docker obrazů, kolik chtějí, aniž by je veřejně vystavovali světu.
...
Harbor využívá funkcionalitu jednotného přihlašování e-infra, takže se můžete snadno přihlásit do této služby pomocí svého účtu Masarykovy univerzity pomocí zobrazeného tlačítka pro přihlášení.
...
Pro nahrání Docker obrazů (nebo Helm chartů) do Harboru budete potřebovat použít příkazový řádek. Protože jednotné přihlášení nelze provést prostřednictvím terminálu, budete se muset přihlásit pomocí tajného klíče, který byl vygenerován při vašem prvním přihlášení.
...
Pokud si přejete, můžete také obnovit své heslo nebo nastavit vlastní kliknutím na tři tečky vedle ikony kopírování.
...
Ve stejné složce, kde máte svůj Dockerfile, spusťte následující příkaz: `docker build -t cerit.io/$USER/tutorial:v0.1 .` (nahraďte `$USER` svým uživatelským jménem v Harboru, bez velkých písmen) (vyberte `cerit.io` nebo `hub.cerit.io` na začátku názvu obrazu na základě toho, který název jste použili v kroku `docker login`)
...
Abychom ukázali, jak to vypadá, když má obraz více tagů, mírně upravíme soubor Dockerfile:
...
(například: architektura, konfigurace a historie sestavení). V tomto přehledu můžeme také skenovat zranitelnosti.
...
V přehledu repozitáře můžeme vidět záložku s názvem `Info`, kde můžeme přidat nějaké informace o obrazu. To může být užitečné ve veřejném projektu, aby lidé věděli, co obraz vlastně dělá. Informace jsou formátovány pomocí [markdown jazyka](https://www.markdownguide.org/).
