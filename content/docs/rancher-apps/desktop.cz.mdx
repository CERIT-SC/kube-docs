...
Můžete spustit desktopovou aplikaci podle níže uvedených kroků. Tato aplikace může mít persistent home directory, což znamená, že pokud aplikaci odstraníte a později ji znovu nainstalujete pod stejným názvem, obsah domovského adresáře bude zachován. Je také možné připojit nějaké úložiště z e-infra.


### Poznámky

* Výchozí projekt měl kvótový limit pro 12 garantovaných CPU a kvótový limit pro 16 CPU. Nevybírejte více než 10 garantovaných CPU a více než 14 CPU limit. Pokud potřebujete více, jednoduše [se zeptejte](mailto:k8s@ics.muni.cz). Toto omezení se může v budoucnu navýšit, takže si o více CPU můžete kdykoli zažádat


### Výběr aplikace ke spuštění

Ujistěte se,  že nemáte vybraný žádný namespace a že v horní části stránky Rancher vidíte `Only User Namespaces` (1).

Přejděte do `App & Marketplace` (2), `Charts` (3), omezte charts pouze na `cerit-sc` (4) a vyberte `desktop` (5). Viz screenshot níže.
...
Když kliknete na chart, můžete vybrat verzi aplikace, jak je znázorněno níže. Verze charts se mohou v průběhu času měnit. Stiskněte `Install` pro pokračování.
...
Nyní jste připraveni nainstalovat desktopovou aplikaci. Většinou necháte jak `Namespace` (1), tak `Name` (2) tak, jak jsou, ale můžete si vybrat jakýkoli namespace kromě `default`. Namespace `default` je k dispozici, ale není určen k použití. `Name` bude součástí URL adresy, přes kterou budete k aplikaci přistupovat. `Name` musí být jedinečné v rámci `Namespace`, tj. nemůžete spustit dvě nebo více instancí se stejným `Name` ve stejném `Namespace`. Pokud aplikaci odstraníte a později ji znovu nainstalujete pomocí stejného `Name`, obsah domovského adresáře bude zachován, pokud je tato možnost povolena.
...
Ve čtvrté části formuláře vyberete, zda chcete persistent home, tj. home, který bude zachován i po odinstalaci aplikace. Poznámka: persistent home je vázán na název aplikace od samého začátku instalačního formuláře. To znamená, že pokud nastavíte název na `desktop`, persistent home bude vázán na název `desktop`. Pokud příště vyberete název `desktop-0`, dostanete nový domovský adresář navázaný na desktop-0.
