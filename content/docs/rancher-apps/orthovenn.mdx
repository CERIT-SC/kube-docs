...
To permanently release the storage, navigate to Rancher’s `Storage` -> `PersistentVolumeClaims` section and manually delete the two associated Persistent Volume Claims (named with the suffixes `-db` and `-home`). 
