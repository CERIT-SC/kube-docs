...
To install Nextflow you need to download and compile our patched version from git:

```shell
git clone https://github.com/CERIT-SC/nextflow.git
cd nextflow
make compile install
```

To run and compile, Java Virtual Machine needs to be installed. On Ubnutu systems, it is package `openjdk-17-jdk:amd64` or newer.

If you want to use *DSL 1* workflows, you need to instal `nextflow` version `22.10.8`:

```shell
git clone https://github.com/CERIT-SC/nextflow-dsl1.git
cd nextflow
make compile install
```

You can compile and install both version simultaneously, if neede.
...
To start a pipeline, you must use the `kuberun` subcommand of `nextflow`: 
...
For *DSL 1* `nextflow`, use image `cerit.io/nextflow/nextflow:22.10.8`:

```shell
nextflow kuberun hello -head-image 'cerit.io/nextflow/nextflow:22.10.8' -head-memory 4096Mi -head-cpus 1 -v PVC:/mnt 
```
...
process {
   ...
   pod = [[label: 'hpctransfer', value: 'must'], [automountServiceAccountToken:false], [secret: 'keythings', mountPath: '/etc/secrets']] // THIS WILL BE APPLIED TO ALL WORKERS
...
Download [nextflow.config](/examples/ceritsc/nextflow/nextflow.config) which is prepared to be used as is, just hange `namespace` to correct value (namespace you can access) and specify `launchDir` and `workDir` to point somewhere on the PVC. Take
...
```shell
nextflow kuberun hello -head-image 'cerit.io/nextflow/nextflow:24.04.4' -v PVC:/mnt 
```
...
```shell
nextflow kuberun nf-core/sarek -head-memory 4096Mi -head-cpus 1 -head-prescript /mnt/nextflow-cfg.sh -head-image 'cerit.io/nextflow/nextflow:24.04.4' -v PVC:/mnt --input /mnt/test.tsv --genome GRCh38 --tools HaplotypeCaller,VEP,Manta
```
...
* Manual resuming of Sarek is possible using different `--input` spec. See [here](https://nf-co.re/sarek/2.7.1/docs/usage#troubleshooting).
...
```shell
nextflow -C nextflow.config kuberun vib-singlecell-nf/vsn-pipelines -head-image 'cerit.io/nextflow/nextflow:24.04.4' -head-cpus 1 -head-memory 4096Mi -v PVC:/mnt -entry scenic
```
