...
Tento příklad umožňuje přístup pouze z konkrétního namespace clusteru Kubernetes (stáhnout zde ). Zadejte název clusteru (pokud byl změněn) jako spec.podSelector.matchLabels.cnpg.io/cluster a nahraďte [namespace] namespace, ve kterém je nasazen PostgreSQL. Ostatní namespace jsou povoleny přidáním dalších položek namespaceSelector do seznamu.
