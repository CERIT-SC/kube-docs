...
That requests a 3-node cluster. This type of setup is resilient to node failure --- if a node running this instance fails, the database is recreated on another node, data is reconnected from NFS storage, and operations resume. On the other hand, this setup is not speed-superior even when resources are added.
...
<Callout type="info">
By default, the example cluster manifests disable the auto-generated read-only services (`<name>-r` and `<name>-ro`). If you want to use read-only replicas, you need **at least 2 instances** and you must **omit** the `spec.managed.services.disabledDefaultServices` section from the cluster manifest.
</Callout>
...
The hostname to use of the read-only replica is `test-cluster-ro` and the hostname of the writable master is `test-cluster-rw`. If not explicitly set otherwise, the port is the well-known PostgreSQL port `5432`.
...
If not explicitly set otherwise, the port is the well-known PostgreSQL port `5432`.
...
If you want read-only access as well (in addition to writable access), two LoadBalancers must be created. In this case, please contact us at **k8s@cerit-sc.cz**.
...
Assuming the database name is again `test-cluster`, the following are examples of needed services for both ro and rw accesses.
...
In this case, the IP addresses are allocated from the public IP pool. When accessing the database from the external world, we **strongly recommend** to setup the Network Policy or the [LoadBalancer firewall](../kubernetes/expose#loadbalancer-firewall).
...
In this case, ensure that the corresponding LoadBalancer Service has the `externalTrafficPolicy` field set to `Local`, i.e., `externalTrafficPolicy: Local`.

If this field is not set or is set to `Cluster`, the network policy will not function as expected. This is because the client source IP address (the one specified in the `ipBlock` → `cidr`) will be masked and replaced by a cluster node's IP address. As a result, it becomes impossible to effectively restrict access based on the original client IP.
</Callout>

<Callout type="warn" icon="⚠️ ">
Network policies must allow ingress from the `cloudnativepg` namespace. Without this access, the operator cannot monitor database pods or perform failover operations.
</Callout>
...
The schedule is **NOT** a standard `cron` schedule -- this seconds/minutes/hours/day of month/month/day of week.

<Callout type="warn">
Some newer Postgres images include updated versions of the `boto3` library, which may be incompatible with certain S3 endpoints (for example, DU CESNET).
When this occurs, the Postgres Pod may log the following error:
...
As a workaround, set the following three environment variables in the cluster definition:
...
Example database cluster definition with the required environment variables:
...
Basically, the `zfs-csi` storage class can be used to use local storage. Special care must be taken when setting the limit. It cannot be increased in the future and the limit is enforced, however, it is the fastest storage that is offered.
...
In the cluster manifest, change the `bootstrap.initdb` section to reference the newly created secret.
...
Currently, CloudNative-PG Docker images do not contain `cs_CZ` locale, so Czech collation cannot be used. For this reason, we have created two local images: `cerit.io/cloudnative-pg/postgresql:10.23-3` and `cerit.io/cloudnative-pg/postgresql:15.0` which contain Czech locales.
