...
### 1. Deployment
Create a new directory, e.g. `hello-world`, and inside, create a new file `deployment.yaml` with content:
```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
...
- `.spec`
  ...
  - `.selector` field defines how the `Deployment` finds which `Pods` to manage. Here, a label defined in the `Pod` template is selected (`app: hello-kubernetes`). It is possible to use sophisticated selection rules, as long as the values `app: hello-kubernetes` match in the `spec:` and `template:` sections of the `Pod` template.
  - `.template`
    ...
    - `.spec` `Pod's` template specification
      ...
      -  `.containers.env` set environment variables for a container. They can be taken from other fields of the object or from other object types (e.g., Secret)

Complete reference documentation for resources and their allowed fields and subfields is available [online](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.30/). Don't forget proper indentation!

### 2. Service
Secondly, we have to create a `Service`, which is an abstract way to expose an application as a network service.
...
A Service can map any incoming port to a targetPort. By default, the targetPort is set to the same value as the containerPort field. The default protocol is TCP, but other supported protocols can also be used (UDP, HTTP, SCTP, PROXY).

### 3. Ingress

Lastly, we have to create an `Ingress`, which exposes HTTP and HTTPS routes from outside the cluster. Traffic is controlled by rules set in the resource.
It is possible to expose your deployments in [two ways](../kubernetes/expose), but here we will use a cluster LoadBalancer with creation of a new DNS name.
You can use whatever name you want but it has to fulfill 2 requirements:
- name is composed only from letters, numbers and '-'
- name ends with `.dyn.cloud.e-infra.cz`

The name is filled in `spec.rules.host` and in `spec.tls`. Before you use any name, check in a browser that it doesn't already exist. After creation, it takes about a minute to create a new DNS entry, so your app will not be available immediately at the specified name; wait at least one minute.
...
- `.metadata`
  - `.name` denotes name
  - `.annotations` ingress frequently uses annotations to configure options depending on ingress controller. We use `nginx` controller and possible annotations are listed [here](https://kubernetes.github.io/ingress-nginx/user-guide/nginx-configuration/annotations/). The ones used here are necessary for right functionality and they automatically create TLS certificate therefore you don't need to worry about HTTPS - it's provided automatically
 - `.spec` includes all information needed to configure a load balancer, but most importantly, includes rules matched against all incoming requests
...
### 4. Create
Now, create all resources using the whole directory as an argument and specify your namespace
```bash
...
You can check the status of deployed resources with `kubectl get [pods | services | ingress] -n [namespace]`. When all of them are up and running, you can access the URL and you will be presented with a sample page.
![hello](/img/ceritsc/hello.png)

NOTE: In case you have trouble with a "Config not found" error message, see [here](../kubernetes/kubectl) for further information.

## Further customization
You can specify various fields in every resource's file, many of which are not used here. One of the more desired features is passing environment variables into `Deployments` in case spawned containers need them. We will use one environment variable in our deployment to change the displayed message. At the end, add a new `env` section, which will forward the value into the pod. Then, run `kubectl apply -f hello-world -n [namespace]` again to apply the changes. When you access the website now, a new message is displayed!
...
Other customizations can include:
- Creating a `Secret` (e.g., for a password) and mounting it into the pod as a file
- Creating a `ConfigMap` (e.g., for larger configurations) and mounting it into the pod
- Creating a `PersistentVolumeClaim` (storage) from NFS (S3 #TODO) and mounting it into the Pod
- Creating resources of other types, e.g., ReplicaSet, StatefulSet, DaemonSet


## Creating PersistentVolumeClaim
If you need to use persistent storage, you can create an NFS volume and mount it in a `Deployment`.
...
- Create a `volumes` field under `spec.template`
- Create a `volumeMounts` field under `spec.template.spec.containers[container_which_will_have_pvc_mounted]`
...
Any deployment that will attempt to run as root won't be created and will persist in state similar to:
```bash
NAME               READY   UP-TO-DATE   AVAILABLE   AGE
hello-kubernetes   0/1     1            0           7m8s
```
 (notice READY 0/3 and AVAILABLE 0, logs and describe would tell more)
...
...
```
- `kubectl get pods -n [namespace]` + `kubectl logs [pod_name] -n [namespace]` shows pod's logs if they were configured or any output occurred. This combination is very useful for debugging.
