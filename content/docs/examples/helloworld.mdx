...
A Service can map any incoming port to a targetPort. By default, the targetPort is set to the same value as the containerPort field. Default protocol is TCP but other supported protocols can be used too (UDP, HTTP, SCTP, PROXY).

### 3. Ingress

Lastly, we have to create `Ingress` which exposes HTTP and HTTPS routes from outside world to the cluster world. Traffic is controlled by rules set in the resource.
It is possible to expose your deployments in [2 ways](../kubernetes/expose) but here we will use cluster LoadBalancer with creation of just new DNS name.
...
You can check status of deployed resources with `kubectl get [pods | services | ingress] -n [namespace]` and when all of them are up and running, you can access the URL and you will be presented with sample page.
