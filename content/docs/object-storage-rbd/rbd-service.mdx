...
### RBD Snapshots 
Snapshots can be created at the RBD (replicated/erasure code) level and are managed from the client side. [RBD snapshotting](rbd-setup) supports rollback of data in case of unwanted rewrite or data corruption. Snapshots can be mirrored to another geographic location, as described below.
