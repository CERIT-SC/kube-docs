...
---
Ceph RBD (RADOS Block Device) offers users a network block device that appears as a local disk on the system where it is connected. The block device is entirely managed by the user, who can create a file system on it and use it according to their needs.

<Callout type="info" emoji="ℹ️">
  **Advantages of RBD**
    * Ability to expand the block device image.
...
<Callout type="info" emoji="ℹ️">
**Ceph client version**
For optimal functionality, it is highly recommended to use the same version of Ceph tools as the one currently running on our clusters. Currently, the version in use is 16, codenamed Pacific. We will set up the appropriate repositories, as outlined below.
</Callout>
