...
**Answer:** This error typically occurs in a Conda environment when the system's OpenSSL version differs from the one used by Conda. The issue occurs because Conda libraries take precedence over system libraries, but certain system binaries (such as `ssh` or `git` which is calling `ssh`) require the system's OpenSSL version to function correctly.

As a temporary workaround, you can unset `LD_LIBRARY_PATH` environment variable before running the problematic command. For example:  
```shell
...
---
**Question:** I'm encountering a connection error such as:  
`Failed to connect to github.com port 443 after 135243 ms: Couldn't connect to server`.

**Answer:**  This error indicates that the Pod does not have access to the external internet. To resolve this, you need to configure an HTTP proxy. You can do this by exporting the following environment variables, either directly in the Pod shell or by setting them in the Pod's deployment manifest under `env`:
...
Some programs (e.g., `wget`) require lowercase `https_proxy` or `http_proxy` variables. In such cases, you also need to export the lowercase variants:
...
<Callout type="warn">  
The proxy is only suitable for HTTP-based protocols. It cannot be used for protocols such as SSH.
</Callout>

<Callout type="warn">
The proxy does not support access to the Kubernetes API. If you export these proxy variables, tools like `kubectl` or `helm` inside the Pod may stop working unless you also configure the `NO_PROXY` environment variable. For example:  
 
```bash
export NO_PROXY=10.43.0.1 # 10.43.0.1 is the exact address of the Kubernetes API
```
</Callout>
