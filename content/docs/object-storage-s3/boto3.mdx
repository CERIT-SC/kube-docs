...
Recent releases of the `boto3` library have encountered issues with third-party S3 storage providers (such as the CESNET S3 storage).<br/> 

These issues are still present in versions `1.36.x` (specifically `1.36.12`). Therefore, we recommend using `boto3` version `1.35.99` or earlier.<br/> 
