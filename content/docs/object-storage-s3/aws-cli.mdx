---
title: AWS CLI as a command-line tool
---
...
In the configuration wizard, make sure to press the space bar when selecting the **Default region name** option. If you don't add a space in the “Default region name” field, the config file will lack the **region** parameter, leading to an **InvalidLocationConstraint** error when using **aws s3**.
</Callout>
...
_Default region name_ - simply press the space bar here!!! Some software tools, such as Veeam, may have specific requirements. In such cases, enter the storage information.<br/>
