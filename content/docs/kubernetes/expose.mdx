...
This chapter explains how to set access to your application from the external network (or "outside"). Althought it is possible to access your application using some IP directly, you should avoid it and follow the instructions below, how to expose the application.
...
1. DNS record `CNAME` is required. For `kuba-cluster`, `CNAME` target is `kuba-pub.cerit-sc.cz`, for `kubh-cluster`, `CNAME` target is `kubh-pub.cerit-sc.cz`.
