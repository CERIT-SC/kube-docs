...
| **Qwen3.5 (397B)** | `qwen3.5` | chat, multimodal, tools | 256k | 32k | fp8 | [Qwen/Qwen3.5-397B-A17B-FP8](https://huggingface.co/Qwen/Qwen3.5-397B-A17B-FP8) |
...
| **Whisper Large v3** (API only) | `whisper-large-v3` | audio, speech recognition | — | — | — | — |

\* **GLM 5.2** natively supports a 1M-token context, but the deployed endpoint is currently capped at **670k tokens** due to a hardware (GPU memory) limitation.

- **GPT-OSS-120B** — 120B language model from OpenAI; strong all-round performance with tool use.
- **DeepSeek V4 Pro** — 1.6T total / ~49B active parameters; excels at mathematics, logical reasoning, and code generation. The base `deepseek-v4-pro` runs with reasoning **off** by default (it can still be enabled explicitly), while `deepseek-v4-pro-thinking` runs with reasoning **on**. Both share the same underlying weights (`deepseek-v4`).
- **GLM 5.2** — 756B model for advanced "agentic" reasoning and multi-step task handling.
- **Kimi K2.7** — large-scale MoE model (~1T parameters) with strong coding performance; multimodal.
- **Qwen3.5 (397B)** — 397B (A17B active) flagship for coding, tool use, and agentic workflows. An int4 (AWQ) build is also served as `qwen3.5-int4` for higher throughput at lower precision.
...
For backward compatibility, several legacy aliases (e.g. `glm-5`, `kimi-k2.6`, `thinker`, `qwen3-coder`, `qwen3-coder-30b`, `qwen3-coder-next`) continue to resolve to their current equivalents.
</Callout>
