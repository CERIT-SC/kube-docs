...
To use this service, you must have a valid **MetaCentrum (e-INFRA CZ) account**.

**How to Get Access**

The CERIT-SC AI services are integrated with the national research infrastructure MetaCentrum. You can apply for access or manage your credentials through the [MetaCentrum registration form](https://metavo.metacentrum.cz/en/application/form).

<Callout type="info">
**Masaryk University students and staff**: If you don't have a MetaCenter account yet, please [request access here](https://signup.e-infra.cz/registrar/?vo=e-infra.cz&group=projects%3Achat-ai&targetnew=https%3A%2F%2Fchat.ai.e-infra.cz&targetexisting=https%3A%2F%2Fchat.ai.e-infra.cz&targetextended=https%3A%2F%2Fchat.ai.e-infra.cz).
Your request will be automatically approved, allowing you to log in and get started immediately.
**Note**: This quick registration provides access only to the AI chat + API services. A standard MetaCentrum account is required for all other Kubernetes services.
</Callout>
...
### Currently Available Models (as of January 26, 2026)

Effective January 26, 2026, the **DeepSeekR1** model will be **superseded** by the **DeepSeek V3.2** model. DeepSeekR1 will no longer be available.
...
| **DeepSeek V3.2**    |  deepseek-v3.2-thinking  | `coder`, `reasoning`, `tool` | A 685B reasoning and instruct (selectable) language model from DeepSeek AI. Excels at complex tasks, including mathematics, logical reasoning, and code generation. | 160k |
...
| Model         |      Model Name         |  Capability  | Description | Context Size |
|---------------|---------------|----------|-------------|--------------|
| **GLM 5**    | glm-5              |  `multimodal`, `coder`, `tool`    | The 744B GLM 5 fp4 language model for advanced "agentic" reasoning and superior multi-step task handling | 200k |
| **Kimi K2.5**  | kimi-k2.5   |  `agentic`, `multimodal`, `coder`, `tool`       |  Large-scale MoE model with roughly 1 trillion parameters with significantly enhanced performance in coding tasks. | 256k   |
| **Qwen 3.5**  | qwen3.5   |   `agentic`, `multimodal`, `coder`, `tool`   |  397B Multimodal advanced model for coding, tool use, and agentic workflows | 262k   |
| **Mistral-small**  | mistral-small-4   |   `multimodal`, `coder`, `tool`  |  397B Multimodal advanced model for coding, tool use, and agentic workflows | 262k   |
...
* Select a text-generation model (e.g., `Gemma 3` or `RedHat AI Scout`)
* Click the **+** icon → **Upload Files** → select an image (JPG/PNG)
...
We prioritize the security of your data. While we maintain strict internal controls, it is essential to understand how your information is processed, stored, and protected within our infrastructure.

<Callout type="warn">
Chat.ai does not have the enhanced security measures required to process sensitive data. **Users are strictly prohibited from uploading highly sensitive, classified, or Personally Identifiable Information (PII) to the WebUI**. While our infrastructure is private, the chat platform is designed for general assistance and research; it is not currently certified for the processing of sensitive/protected documents.
</Callout>
...
### Model Infrastructure & Data Flow
...
We minimize data retention through the following logging policies:
