...
description: Comprehensive guide to using Open WebUI chat interface with large language models on the e-INFRA CZ AI platform
---
...
| **GPT OSS 120B with Memory** |  | The 120B GPT OSS model for retaining key facts (e.g., preferences, project details). Enabling the memory feature personalizes responses across conversations. To enable `Memory`, click your username (bottom-left), go to `Settings` → `Personalization` → `Memory` tab, and toggle `Memory` on. To add memories, use either method: 1) **Manage Button**: In the Memory tab, click Manage to add/edit memories (e.g., "My project: CFD optimization") or 2) **Chat Command**: During chats, type "remember [fact]" (e.g., "remember my MetaCentrum login is xnovak12"). The model confirms storage. *Important: When enabled, memories are used across all supported models.*| 128k |
...
| **Qwen 3.5**  | qwen3.5   |  397B Multimodal advanced model for coding, tool use, and agentic workflows | 262k   |
...
## Creating an API Key {#model-names-for-api-interface}
