...
3. Select `e-INFRA CZ AAI` and choose `e-INFRA CZ password` from the list of organizations. Log in using your MetaCentrum credentials.
...
2. Name your instance: Enter the `Namespace` (username-ns) and a unique `Name` for your deployment (e.g., username-n8n). Remember these values and click `Next`.
...
1. Open your browser and navigate to the Access URL saved in the previous step.
...
1. Open https://chat.ai.e-infra.cz/ in your browser.
2. Open the `Settings` menu → `Account` → `API keys`.
3. Create a new API key and copy the generated token.
