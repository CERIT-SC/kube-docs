...
BinderHub is a Binder instance running on Kubernetes at [binderhub.cloud.e-infra.cz](https://binderhub.cloud.e-infra.cz/). Binder turns a Git repository into a collection of interactive notebooks. Enter the Git repository name (and optionally a specific notebook or branch), and BinderHub will convert it into a web notebook.
...
Lines starting with `#` are ignored (useful for comments); other stand for:
