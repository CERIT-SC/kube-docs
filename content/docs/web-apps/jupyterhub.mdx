...
<Callout type="warning"> 
Please be aware that Claude Code integration relies on browser-based storage. In the event of a session timeout due to inactivity or if the browser window is closed, this context may be purged, causing the assistant to become unresponsive in Jupyter Notebook, RStudio, or VS Code.
