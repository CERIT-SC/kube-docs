...
Flare tunnel umožňuje zpřístupnění vašich lokálních aplikací na doméně `flare.cloud.e-infra.cz`.
...
```
kde `{port}` je port, na kterém běží vaše aplikace, a `{name}` určuje URL, na kterém bude aplikace dostupná.  
