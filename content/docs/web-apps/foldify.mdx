...
### 2. Computations Table Section
This section consists of a table listing both the last computed jobs and currently running jobs with their state.
...
### 3. Example Sequences Section
This section contains precomputed protein sequences that demonstrate the use-cases of the Foldify platform. You can show details of each example by clicking the `View Details` button.
...
If you want to predict the same protein structure with multiple tools for comparison purposes, you can use the `Multifold` feature. To do so, choose the `Multifold` card in the dashboard.
