...
By default, all calculation results are private, i.e. only you can see them. If you want to share your results with collegues, you can do so by checking (*Make results public* checkbox). In this case, the results is shown in `Public Jobs Table` and you can share the computation with link.
...
To run a computation, go to the Dashboard View and select the tool you want to use. Click the `Run Job` button and fill in the parameters for the computation. After that, click the `Run Job` button again to start the computation.

### How to view the results?
To view the results, go to the Dashboard View and find the protein name in the `Last Computed Jobs` table. Click the `View Result` button to display the result in the browser.

### How to share the results?
To share the results, check the `Make results public` checkbox when running the computation. This will make the results visible in the `Public Jobs` table and you can share the link with your collegues.

### How to download the results?
To download the results, click the `Download` button in the `Last Computed Jobs` table. Please note that the results are in most cases large files and can take some time to download. After closing the dialog window downloading process will be cancelled.
