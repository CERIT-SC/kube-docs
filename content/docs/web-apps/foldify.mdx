...
Foldify supports the use of MMSeqs2 for faster predictions. MMseqs2 is a fast and sensitive sequence search tool that can significantly speed up the prediction process. We use the scripts to add MMseqs2 MSA/templates to an AlphaFold 3 input JSON file.

To use MMseqs2, you need to check the `Use Generated MSA` checkbox in the form - it is checked by default. This will enable the use of MMseqs2 for faster predictions. If you want to specify number of templates, check the `Use Generated Templates` checkbox and specify the number of templates in the `Number of Templates` field that appears. The default value is 20, but you can change it to custom value.
