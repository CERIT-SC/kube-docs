...
If **no GPU** was requested, the mechanism checks only CPU usage. After 4 days of &lt;0.01 CPU usage, the notebook instance is deleted. The threshold was chosen based on data collected by monitoring --- CPU usage below 0.01 suggests that the notebook just exists and no computation is performed.
...
Additionally, you need to add your *public* ssh key into home `.ssh/authorized_keys` file. If the directory `.ssh` does not exist, just create it using `mkdir .ssh`. Do not omit the dot at the beginning of the name. Also note, `ls` does not display doted files, to see `.ssh` directory, you need to use `ls -a`. 

After adding the public ssh key, you should be able to connect with ssh similarly to `ssh jovyan@jupyter-xhejtman---54est.dyn.cloud.e-infra.cz`.
