...
Where `selector:` `app: application` must match the application name in the deployment and `application-svc` is any name. If annotation `external-dns.alpha.kubernetes.io/hostname` is provided and value is in domain `dyn.cloud.e-infra.cz`, the name will be registered in DNS.
