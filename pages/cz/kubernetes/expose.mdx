...
Objekt Ingress může vystavit aplikace, které používají protokol HTTPS místo HTTP, tj. komunikace mezi ingress a aplikací je také šifrována. V tomto případě je nutné přidat anotaci `nginx.ingress.kubernetes.io/backend-protocol: "HTTPS"` k objektu ingress.
